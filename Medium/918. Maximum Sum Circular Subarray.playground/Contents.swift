...
class Solution {
    func maxSubarraySumCircular(_ A: [Int]) -> Int {
        ...
        //handle the case for all negative elements
        var negCount : Int = 0
        for elem in A{
            if elem < 0 {
                negCount += 1
        ...
        // 1. find kadane on original Array
        var kadaneOriginal = kadane(A)

        // 2. find arr Sum
        var arrSum : Int = A.reduce(0, +)

        // Negate all the elements of A
        var newArr : [Int] = [Int]()
        for elem in A{
            newArr.append(-elem)
        }

        // 3. Find kadane of negated arr
        var kadaneNegated = kadane(newArr)

        //result is max(1,2+3)
        return max(kadaneOriginal, arrSum + kadaneNegated)
    }
}
